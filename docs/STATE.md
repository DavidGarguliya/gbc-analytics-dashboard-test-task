...
## In progress
- No active coding tasks. `task/readme-russian-handoff` is locally verified and ready to promote through `feat/next-stage-baseline` into `main`.
...
- redeploy the dashboard only if a later code slice changes runtime behavior; the current README refresh does not require a new deployment
